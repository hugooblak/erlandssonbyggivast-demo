...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'erlandssonbyggivast-leads' ),
					'singular_name' => __( 'Lead', 'erlandssonbyggivast-leads' ),
					'edit_item'     => __( 'Förfrågan', 'erlandssonbyggivast-leads' ),
					'search_items'  => __( 'Search leads', 'erlandssonbyggivast-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'erlandssonbyggivast-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'erlandssonbyggivast-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'erlandssonbyggivast-leads' ),
			'npl_phone'    => __( 'Phone', 'erlandssonbyggivast-leads' ),
			'npl_service'  => __( 'Needs', 'erlandssonbyggivast-leads' ),
			'npl_timeline' => __( 'When', 'erlandssonbyggivast-leads' ),
			'npl_zip'      => __( 'ZIP', 'erlandssonbyggivast-leads' ),
			'npl_source'   => __( 'Source', 'erlandssonbyggivast-leads' ),
			'npl_status'   => __( 'Status', 'erlandssonbyggivast-leads' ),
			'date'         => __( 'Received', 'erlandssonbyggivast-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'erlandssonbyggivast-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'erlandssonbyggivast-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'erlandssonbyggivast-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'erlandssonbyggivast-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'erlandssonbyggivast-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'erlandssonbyggivast-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'erlandssonbyggivast-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'erlandssonbyggivast-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'erlandssonbyggivast-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'erlandssonbyggivast-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'erlandssonbyggivast-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'erlandssonbyggivast-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
