 ...
 * or photo. They call er_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Erlandsson Bygg i Väst AB.
 *
 ...
 * @return string
 */
function er_sample_note() {
	return er_lead( 'ar_riktig' ) ? '' : ' <span class="er-sample">(platshållare)</span>';
}
 ...
 * @return string File name for er_lead_img().
 */
function er_lead_bild( $i ) {
	$bilder = (array) er_lead( 'bilder', array() );
 ...
 * @return string
 */
function er_ring_label() {
	$namn = er_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, er_lead( 'telefon' ) ) : 'Ring ' . er_lead( 'telefon' );
}
