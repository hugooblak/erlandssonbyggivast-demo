...
add_filter(
	...
	function ( $exporters ) {
		$exporters['erlandssonbyggivast-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'erlandssonbyggivast-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'erlandssonbyggivast-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'erlandssonbyggivast-leads',
						'group_label' => __( 'Quote requests', 'erlandssonbyggivast-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['erlandssonbyggivast-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'erlandssonbyggivast-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
